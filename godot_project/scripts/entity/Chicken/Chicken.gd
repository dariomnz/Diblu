...
onready var animationPlayer : = $Chicken_animation
...
func _physics_process(delta):
	...
	if is_escape_to:
		velocity = (is_escape_to.position - position).normalized() * 40
		velocity*=-1
...
func _on_HP_bar_death():
	...
	if rand.randi()%100 <30:
		global.spawn_item(position,"golden_feather")
	else:
		global.spawn_item(position,"feather")
